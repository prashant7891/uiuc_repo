...
\subsubsection{Planet Labs’ Remote Sensing Satellite System (2013)}
...
space imagery requirements.\cite{Stepan_ConstellationImagery}


\subsubsection{Simultaneous Multi-Point Space Weather Measurements using the Low
Cost EDSN CubeSat Constellation (2013)}
...
fly late 2013 as secondaries on a DoD mission.\cite{Gunderson_MultiptMeasureConstellation}


\subsubsection{Small Satellite Constellations for Earth Geodesy and Aeronomy (2013)}
...
and its impact on all satellites that orbit the Earth.\cite{Conklin_GeoAero_Constellation}
...
and retrieval techniques of the temperature and humidity will be discussed.\cite{Sharmila_ContellationTempHumidity}


\subsection{Formation Flying Small Satellites}


\subsubsection{NASA's Four Spacecraft Magnetospheric Multiscale Mission (2013)}
...
Launch will be in October 2014 on an Atlas-V 421 Launch Vehicle.\cite{Klumpar_4FF_spacecraft}.


\subsubsection{EDSN Update (2013)}
...
development within that project.\cite{Yost_MultipointMeasurement}


\subsubsection{The CanX-4\&5 Formation Flying Mission: A Technology Pathfinder for
Nanosatellite Constellations (2013)}
...
applications.\cite{Bonin_FF_CanX-4&5}


\subsubsection{Small Satellite Cluster Inter-Connectivity (2013)}
...
protocols for distributed small satellite networks.\cite{Radhika_FF_intercomunication}
...
and real-time station keeping will be discussed. \cite{Voronka_FF_Crosslink}


\subsubsection{From Single to Formation Flying CubeSats: An Update from the Delft
Programme (2013)}

\cite{Guo_FF_single2FF} Currently, most Cubesat missions are used for technology demonstrations
...
The Orbiting Low Frequency Antennas for Radio Astronomy (OLFAR) project
is investigating an orbiting low frequency radio telescope. Due to
strong ionospheric interference and Radio Frequency Interference (RFI)
found at frequencies below 30 MHz, such an instrument is not feasible
on Earth, hence the proposed solution of a swarm of autonomous nano-satellites
sent to a remote location in space. On each satellite, the astronom-
ical antenna consists of three orthogonal dipoles designed to work
within the constraints of a nano-satellite. Due to mechanical constraints,
the dipoles are not optimally integrated into the nano-satellites
from an antenna point of view. Unfortunately, the operational band
of 0:3 MHz to 30 MHz and the dimensions of the astronomical antenna
of just under 5:0 m prohibit tests within the controlled environment
of an anechoic chamber; ergo, a scale model is required. This work
describes the design, simulation and measurement of such a scale model.\cite{Quillen_FF_RadioTelescope}


\subsubsection{Design of Nano-satellite Cluster Formations for Bi-Directional Reflectance
Distribution Function (BRDF) Estimations (2013)}
...
are shown to be achievable using CubeSat reaction wheels.\cite{Nag_BRDF}
...
formation flight via differential drag.\cite{Gangestad_FF_AeroForces}
...
Mitigation through Launch of Space Tether in Low Earth Orbits (2013)}
...
commercial off the shelf components.\cite{Asundi_SpaceDebris}


\subsubsection{CubeSats – Have They Reached Their Explorer-1 Moment? (2013)}
...
future. \cite{Freeman_Exoplanet}


\subsubsection{PanelSAR: A Smallsat Radar Instrument (2013)}
...
development of the smallsat miniSAR is addressed.\cite{Dujin_Radar}
...
(SWIR) to long wave infrared (LWIR) bands. \cite{Champagne_deployablemirror}
...
thermal isolation, heat dissipation, and EMI shielding.\cite{Wrobel_Shielding}
...
with distributed apertures. \cite{Elwood_CollapsibleTelescope}


\subsubsection{Asteroid Prospector (2013)}
...
details of the control and guidance algorithms. \cite{Muelle_AsteroidProspector}


\subsubsection{FTS CubeSat Constellation Providing 3D Winds (2013) }
...
will also be discussed. \cite{Wloszek_3Dwinds}


\subsubsection{Real-Time Geolocation with a Satellite Formation (2013)}
...
recursive filtering techniques.\cite{Leiter_Geoloc}
...
Satellites: A Revolution in Space Science (2013)}
...
identified to enable these new classes of missions. \cite{Norton_KISS}


\subsubsection{The Next Little Thing: Femtosatellites (2013)}
...
\cite{Janson_Femtosats}


\subsubsection{Autonomous Assembly of a Reconfiguarble Space Telescope (AAReST)
– A CubeSat/Microsatellite Based Technology Demonstrator (2013)}
...
fixed mirrors and a boom-deployed focal plane assembly. \cite{Underwood_ReconfigTelescope}


\subsubsection{NASA’s GRAIL Spacecraft Formation Flight, End of Mission Results,
and Small-Satellite Applications (2013)}
...
mission Science phases. \cite{Edward_GRAIL}
